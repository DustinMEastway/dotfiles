...
const convert = () => {
  const homeDirectory = os.
  const inputPath: string = argv[3] ?? '/Library/Application\ Support/Sublime\ Text/Packages/User/snippets';
  const outputDirectory: string = argv[4] ?? '/Library/Application\ Support/Code/User/snippets' ;
