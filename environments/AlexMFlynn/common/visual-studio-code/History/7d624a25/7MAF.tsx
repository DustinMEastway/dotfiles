...
export const Drawer: FC<Overlay | Push> = ({
  ...
  // const [parent, setParent] = useState((parentId) ? document.getElementById(parentId) : null);
  
  // useEffect(() => {
  //   setParent((parentId) ? document.getElementById(parentId) : null);
  // }, [parentId]);

  const parent = useMemo(() => {
    const newParent = document.getElementById(parentId ?? '');
    console.log('newParent', newParent, parentId);
    return newParent;
  }, [parentId]);
